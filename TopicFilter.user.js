...
// @version      1.0
// @description  Filter Topic,Only available on selected sites
// @author       AM
// @match        https://*/thread_new.php*
...
function adddivhtml() {
    ...
    addhtml += '</div>';
    ndiv.innerHTML = addhtml;
    //bodyhtml.innerHTML = bodyhtml.innerHTML.concat(addhtml);
    body.insertBefore(ndiv,body.lastChild)
}
(function () {
    ...
    if (tb != null || tb != '') {
        ...
        tb = document.getElementById('ajaxtable');//取出对应列表
        let trtd, tbt, tbcon;
        if (tb == null || tb == '') {
            log('search');
            ...
            for (let i = tb.rows.length - 1; i >= 0; i--) {
                //对应列
                trtd = tb.rows[i].cells[1];
                if (is_notundefined(trtd)) {
                    //取出h3标签数据
                    tbt = trtd.getElementsByTagName("h3")[0];
...
function xuser(uth) {
    if (is_notundefined(uth)) {
        let atag = uth.getElementsByTagName("a")[0];
        //log(atag+'--------------'+atag.innerHTML);
        if (is_xuse(atag.innerHTML)) {
            return true;
        } else {
            //log(uid);
            return is_xuse(get_uid(atag.href));
        }
    }
}
